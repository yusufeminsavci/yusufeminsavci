...
public class Main {
    public static <book> void main(String[] args) {
        ...
        bookList.stream().forEach(book->bookMap.put(book.getClass(), book.getClass()));
        for(String s:bookMap.keySet()){
            System.out.println("Book Name: "+s+"\nAuthorName: "+bookMap.get(s));
        }
        System.out.println("*********************************************");
        List<book> listBook;
        listBook = bookList.stream().filter(book->book.getClass()>100).collect(Collectors.toList());
        listBook.stream().forEach(b->System.out.println("Kitap Adi : "+b.getClass()+" | " +
                "Sayfa Sayisi : "+b.getClass()+" | "+
                "Yazar : "+b.getClass()+" | "+
                "Yayin Tarihi : "+b.getClass()));
    }
}
